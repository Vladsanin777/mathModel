...
int main() {
            ...
            if (findIntersection(lines[i], lines[j], intersection)) {
                if (checkConstraints(intersection.x1, intersection.x2)) {
                    ...
                    for (const auto& v : vertices) {
                        if (std::abs(v.x1 - intersection.x1) < 1e-4 && 
                            std::abs(v.x2 - intersection.x2) < 1e-4) {
                            isDuplicate = true;
                            break;
                        }
                    }
    ...
    double minL = calculateL(vertices[0]);
    ...
    std::cout << "The minimum is reached at point (" << minPoint.x1 << "; " << minPoint.x2 << ")\nL_min = " << minL << std::endl;
